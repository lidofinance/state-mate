...
- Seed configs use a `.seed.yml` suffix and are consumed by `--generate`, which writes a sibling `*.seed.generated.yml`. `--generate` only downloads ABIs when combined with `--update-abi-missing` / `--update-abi`.
