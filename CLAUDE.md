...
- `yarn test` — cross-repo comparison harness (`src/test-util/app.ts`); `yarn test:unit` — standalone unit tests (`src/test-util/*.test.ts`: `deployed-addresses.test.ts`, `inputs.test.ts`, plus `cli-siblings.test.ts`, which spawns the real CLI to cover the `--deployed`/`--inputs` wiring).
